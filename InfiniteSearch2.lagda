...
head-predicate-same-mod-attempt : ((T , cs) : sequence-of-clofun-types)
                                → (is : (n : ℕ) → is-clofun (cs n))
                                → (Is : (n : ℕ) → c-searchable (T n) (cs n))
                                → ((p , d) : d-predicate (Π T))
                                → (δ : ℕ) → (ϕ : succ δ is-u-mod-of p on (Π-clofun (T , cs)))
                                → succ δ is-u-mod-of pr₁ (head-predicate-attempt (T , cs) is Is (p , d) δ ϕ) on (cs 0)
head-predicate-same-mod-attempt (T , cs) is Is (p , d) δ ϕ (x , y) δ≼cxy
 = ϕ (x :: 𝓔xs x , y :: 𝓔xs y) (build-up (T , cs) is x y (𝓔xs x) (𝓔xs y) δ δ≼cxy TODO)
  where
    𝓔xs : T 0 → Π (T ∘ succ)
    𝓔xs x = Searcher-attempt (T ∘ succ , cs ∘ succ) (is ∘ succ) (Is ∘ succ)
              (tail-predicate (p , d) x)
...
head-predicate-full-attempt : ((T , cs) : sequence-of-clofun-types)
                            → ((n : ℕ) → is-clofun (cs n))
                            → (Is : (n : ℕ) → c-searchable (T n) (cs n))
                            → ((p , d) : d-predicate (Π T))
                            → (δ : ℕ) → succ δ is-u-mod-of p on (Π-clofun (T , cs))
                            → uc-d-predicate (T 0) (cs 0)
head-predicate-full-attempt (T , cs) is Is (p , d) δ ϕ
 = head-predicate-attempt (T , cs) is Is (p , d) δ ϕ
 , succ δ
 , head-predicate-same-mod-attempt (T , cs) is Is (p , d) δ ϕ

\end{code}

We define the Searcher and Condition as before.
...
However, there is one key difference -- the hole marked 'TODO'.
...
type is trivially continuous. It turns out filling this hole it is not
...
tail-predicate-agree : ((T , cs) : sequence-of-clofun-types)
                     ...
                     → (ϕ₂ : succ δ is-u-mod-of p₂ on (Π-clofun (T , cs)))
                     → agree-everywhere (tail-predicate (p₁ , d₁) x) (tail-predicate (p₂ , d₂) y)
tail-predicate-agree (T , cs) is (p₁ , d₁) (p₂ , d₂) δ x y δ≼cxy (f , g) ϕ₁ ϕ₂
 = (λ xs pₜ₁xs → ϕ₂ (x :: xs , y :: xs) (build-up (T , cs) is x y xs xs δ δ≼cxy (δ≼cxsxs xs)) (f (x :: xs) pₜ₁xs))
 , (λ xs pₜ₂xs → ϕ₁ (y :: xs , x :: xs) (build-up (T , cs) is y x xs xs δ δ≼cyx (δ≼cxsxs xs)) (g (y :: xs) pₜ₂xs))
 where
...
In order to prove this clearly, we reformulate the proof that 𝟚
is continuously searchable.
...
𝟚-has-agreeable-searcher : agreeable (discrete-clofun 𝟚-is-discrete) 𝟚-is-c-searchable
...
Agreeable : ((T , cs) : sequence-of-clofun-types)
          → (is : (n : ℕ) → is-clofun (cs n))
          → (Is : (n : ℕ) → c-searchable (T n) (cs n))
          → (ccs : (n : ℕ) → agreeable (cs n) (Is n))
          → ((p₁ , d₁) (p₂ , d₂) : d-predicate (Π T))
...
head-predicate-same-mod : ((T , cs) : sequence-of-clofun-types)
                        → (is : (n : ℕ) → is-clofun (cs n))
                        → (Is : (n : ℕ) → c-searchable (T n) (cs n))
                        → (ccs : (n : ℕ) → agreeable (cs n) (Is n))
                        → ((p , d) : d-predicate (Π T))
                        → (δ : ℕ) → (ϕ : succ δ is-u-mod-of p on (Π-clofun (T , cs)))
                        → succ δ is-u-mod-of pr₁ (head-predicate (T , cs) is Is ccs (p , d) δ ϕ) on (cs 0)
head-predicate-same-mod (T , cs) is Is ccs (p , d) δ ϕ (x , y) δ≼cxy
 = ϕ (x :: 𝓔xs x , y :: 𝓔xs y) (build-up (T , cs) is x y (𝓔xs x) (𝓔xs y) δ δ≼cxy TODO)
  where
    𝓔xs : T 0 → Π (T ∘ succ)
    𝓔xs x = Searcher (T ∘ succ , cs ∘ succ) (is ∘ succ) (Is ∘ succ) (ccs ∘ succ)
              ...
              δ (tail-predicate-reduce-mod (T , cs) is (p , d) x δ ϕ)
    TODO : (δ ↑) ≼ Π-clofun (T ∘ succ , cs ∘ succ) (𝓔xs x , 𝓔xs y)
    TODO = Agreeable (T ∘ succ , cs ∘ succ) (is ∘ succ) (Is ∘ succ) (ccs ∘ succ)
            (tail-predicate (p , d) x)
...
head-predicate-full : ((T , cs) : sequence-of-clofun-types)
                    → ((n : ℕ) → is-clofun (cs n))
                    → (Is : (n : ℕ) → c-searchable (T n) (cs n))
                    → ((n : ℕ) → agreeable (cs n) (Is n))
                    → ((p , d) : d-predicate (Π T))
                    → (δ : ℕ) → succ δ is-u-mod-of p on (Π-clofun (T , cs))
                    → uc-d-predicate (T 0) (cs 0)
head-predicate-full (T , cs) is Is ccs (p , d) δ ϕ
 = head-predicate (T , cs) is Is ccs (p , d) δ ϕ
 , succ δ
 , head-predicate-same-mod (T , cs) is Is ccs (p , d) δ ϕ

\end{code}

We must also show that two head preds are agreeable.
...
head-predicate-agree : ((T , cs) : sequence-of-clofun-types)
                     ...
                     → (ϕ₁ : succ δ is-u-mod-of p₁ on (Π-clofun (T , cs)))
                     → (ϕ₂ : succ δ is-u-mod-of p₂ on (Π-clofun (T , cs)))
                     → agree-everywhere (head-predicate (T , cs) is Is ccs (p₁ , d₁) δ ϕ₁)
                                        (head-predicate (T , cs) is Is ccs (p₂ , d₂) δ ϕ₂)
head-predicate-agree (T , cs) is Is ccs (p₁ , d₁) (p₂ , d₂) δ (f , g) ϕ₁ ϕ₂
 ...
 where
   ...
   γ x = build-up (T , cs) is x x (𝓔xs₁ x) (𝓔xs₂ x) δ δ≼cxx
           (Agreeable (T ∘ succ , cs ∘ succ) (is ∘ succ) (Is ∘ succ) (ccs ∘ succ)
             (tail-predicate (p₁ , d₁) x) (tail-predicate (p₂ , d₂) x)
             δ
             (tail-predicate-agree (T , cs) is (p₁ , d₁) (p₂ , d₂) δ x x δ≼cxx (f , g) ϕ₁ ϕ₂)
             (tail-predicate-reduce-mod (T , cs) is (p₁ , d₁) x δ ϕ₁)
             (tail-predicate-reduce-mod (T , cs) is (p₂ , d₂) x δ ϕ₂))
    where
...
Searcher  (T , cs) is Is ccs (p , d) (succ δ) ϕ
 = x :: 𝓔xs x
 where
   pₕ = pr₁ (head-predicate (T , cs) is Is ccs (p , d) δ ϕ)

   S-head : Σ x₀ ꞉ T 0 , (Σ pₕ → pₕ x₀)
   S-head = Is 0 (head-predicate-full (T , cs) is Is ccs (p , d) δ ϕ)
...
Condition (T , cs) is Is ccs (p , d) (succ δ) ϕ (α , pα)
 ...
 where
   ...
   γₜ : (x' : T 0) → Σ (pₜ x') → pₜ x' (𝓔xs x')
   γₜ = λ x → Condition (T ∘ succ , cs ∘ succ) (is ∘ succ) (Is ∘ succ) (ccs ∘ succ)
               (tail-predicate (p , d) x)
               δ (tail-predicate-reduce-mod (T , cs) is (p , d) x δ ϕ)
...
The final part of the Tychonoff theorem is to prove that Tychonoff is Agreeable.

\begin{code}

Agreeable (T , cs) is Is ccs (p₁ , d₁) (p₂ , d₂) 0        (f , g) ϕ₁ ϕ₂
  = ≼-clofun-refl (Π-clofun (T , cs)) (Π-is-clofun (T , cs) is)
      0 (Searcher (T , cs) is Is ccs (p₁ , d₁) 0 ϕ₁)

Agreeable (T , cs) is Is ccs (p₁ , d₁) (p₂ , d₂) (succ δ) (f , g) ϕ₁ ϕ₂
 = build-up (T , cs) is x y (𝓔xs₁ x) (𝓔xs₂ y) δ γ₁ γ₂
 where
   x y : T 0
   x = pr₁ (Is 0 (head-predicate-full (T , cs) is Is ccs (p₁ , d₁) δ ϕ₁))
   y = pr₁ (Is 0 (head-predicate-full (T , cs) is Is ccs (p₂ , d₂) δ ϕ₂))
   𝓔xs₁ 𝓔xs₂ : T 0 → Π (T ∘ succ)
   𝓔xs₁ x = Searcher (T ∘ succ , cs ∘ succ) (is ∘ succ) (Is ∘ succ) (ccs ∘ succ)
               (tail-predicate (p₁ , d₁) x)
               δ (tail-predicate-reduce-mod (T , cs) is (p₁ , d₁) x δ ϕ₁)
   ...
   γ₂ = Agreeable (T ∘ succ , cs ∘ succ) (is ∘ succ) (Is ∘ succ) (ccs ∘ succ)
          (tail-predicate (p₁ , d₁) x)
          (tail-predicate (p₂ , d₂) y)
          δ
...
ℕ→ℕ→𝟚-is-c-searchable' = tychonoff
                           ...
                           (λ _
                            → Agreeable ((λ _ → 𝟚) , (λ _ → discrete-clofun 𝟚-is-discrete))
                                          (λ _ → discrete-is-clofun 𝟚-is-discrete)
                                          (λ _ → 𝟚-is-c-searchable)
                                          (λ _ → 𝟚-has-agreeable-searcher))
